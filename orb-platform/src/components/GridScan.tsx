...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float scanColorMix = clamp(hit.x + 0.5, 0.0, 1.0);
  vec3 beamColor = mix(uScanColor, uScanColorAlt, scanColorMix);
  vec3 scanCol = beamColor * combinedPulse;
  vec3 scanAura = beamColor * combinedAura;
...
export const GridScan = ({
  ...
  const skewScale = s === 0 ? 0 : THREE.MathUtils.lerp(0.06, 0.2, s);
  const tiltScale = s === 0 ? 0 : THREE.MathUtils.lerp(0.12, 0.3, s);
  const yawScale = s === 0 ? 0 : THREE.MathUtils.lerp(0.1, 0.28, s);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (scanColors.length > 0) {
        const cycle = Math.floor((now / 1000) / Math.max(0.05, scanDuration + scanDelay));
        const cycleColor = scanColors[cycle % scanColors.length];
        const color = srgbColor(cycleColor);
        material.uniforms.uScanColor.value.copy(color);
        material.uniforms.uScanColorAlt.value.copy(color);
      }
